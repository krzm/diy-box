	...
	public class MarkSheetVerticallyFront : IDescriptor
	{
		public string GetDescription(object data)
		{
			...
			if(info.Waste.IsFrontWaste)
			{
				...
				sb.AppendLine($"Next mark at {info.Box.Front.Fold.Y} down.");
				sb.AppendLine($"Next mark at {info.Box.Front.Wall.Y} down.");
				sb.AppendLine($"Check if remaining length is {info.Box.Front.Fold.Y}.");
				sb.AppendLine($"Check down again, if remaining length is {info.Waste.WasteHeight}.");
				sb.AppendLine("Mark it with X as a waste.");
			}
			else
			{
				sb.AppendLine($"Next go to top, left line and mark {info.Box.Front.Fold.Y} from top going down.");
				sb.AppendLine($"Next mark at {info.Box.Front.Wall.Y} down.");
				sb.AppendLine($"Check if remaining length is {info.Box.Front.Fold.Y}.");
			}
			return sb.ToString();
